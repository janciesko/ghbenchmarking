...
void run_gups(GUPSDeviceArray& indices, GUPSDeviceArray& data,
  ...
  using team_t = Kokkos::TeamPolicy<DefaultExecSpace>::member_type;
  ...
  Kokkos::parallel_for(
    ...
    KOKKOS_LAMBDA(const Kokkos::TeamPolicy<DefaultExecSpace>::member_type &team) {
      ...
      Kokkos::parallel_for(Kokkos::TeamThreadRange(team, first_i, last_i), [&](const int64_t j){
        ...
        const int64_t last_thread_i  = first_thread_i + iters_per_thread< last_i
                                     ...
                                     : last_i;
              Kokkos::parallel_for(
              Kokkos::ThreadVectorRange(team,last_thread_i - first_thread_i), [=](const int64_t i) {
